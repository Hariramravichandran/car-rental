...
async def createuser(firstname, lastname, phonenumber, email,guser,address,usertype):
        ...
        async with db.pool.acquire() as connection:
                async with connection.transaction():
                    number_exist = await connection.fetch(
                        'SELECT phonenumber FROM public.users WHERE phonenumber = $1',
                        phonenumber,
                    )
                    ...
                    user = await connection.fetch(
                        """
                        INSERT INTO public.users (firstname, lastname, phonenumber, email, guest,address,usertype)
                        VALUES ($1, $2, $3, $4, $5,$6)
                        RETURNING id
                        """,
                        firstname,
...
async def update_user(id, firstname, lastname, phonenumber, email,address):
    """
    ...
    try:    
        async with db.pool.acquire() as conn:
            async with conn.transaction():
                    ...
                    user = await conn.fetch("SELECT id, CONCAT(firstname, ' ', lastname) AS name, email, phonenumber,usertype FROM users where id=$1", user)
                    return user
